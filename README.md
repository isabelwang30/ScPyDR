...
Using the `-u` and `-v` flags, this should produce outputs `bench_pca.txt` containing a matrix of the original data fitted to the new PCs, and `bench_pca_plot.png`, a PCA plot of the top 2 principal components that explain the most variance in the data. A subset of the first row of bench_pca.txt (produced with `cat bench_pca.txt | head -n 1`) is shown below:
...
The plots for PCA and UMAP from the benchmark dataset are shown below. *NOTE:* because UMAP is a stochastic process, the graph may have global differences. In other words, the local clusters will look similar, but they may be placed in different locations on the plot.

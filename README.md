...
The input file should be a count matrix from the output from the `10X cellranger`'s cellranger count analysis pipeline. A widely-used source to publish and find count matrix scRNA-seq data is GEO. Benchmarking data for this package was found on GEO. Read more on the file format [here](https://www.10xgenomics.com/support/software/cell-ranger-arc/latest/analysis/feature-barcode-matrices). To run scPyDR, the barcodes and features matrices should be loaded as gzip files and placed into a single directory. The name of this directory (as a `str`) is used as the input to scPyDR.
